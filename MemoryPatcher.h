...
public:
	...
	static DWORD GetFunctionAddress(LPCSTR szLibrary, char * szFunction)
	{
		return (DWORD)GetProcAddress(LoadLibrary(szLibrary), szFunction);
	}

	static DWORD GetFunctionAddress(LPCSTR szLibrary, unsigned int uOrdinal)
	{
		return GetFunctionAddress(szLibrary, (char *)MAKELONG(uOrdinal, 0));
	}

	static void * InstallDetourPatch(LPCSTR szLibrary, char * szFunction, DWORD dwFunctionAddress)
	{
		return InstallDetourPatchInternal(GetFunctionAddress(szLibrary, szFunction), dwFunctionAddress, X86_JMP, 5);
	}

	static void * InstallDetourPatch(LPCSTR szLibrary, unsigned int uOrdinal, DWORD dwFunctionAddress)
	{
	...
	static BYTE InstallDetourPatchWithData(LPCSTR szLibrary, unsigned int uOrdinal, DWORD dwFunctionAddress)
	{
